...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("fusion_lock")) {
            resId = R.string.lockscreen_lock_icon_fusion;
    ...
    private void resizeFusionLock() {
        Bitmap fusionLock = BitmapFactory.decodeResource(getResources(), R.drawable.fusion_lock);
        if (fusionLock != null) {
            String path = null;
            int px = requestImageSize();
            fusionLock = Bitmap.createScaledBitmap(fusionLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "fusion_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                fusionLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
